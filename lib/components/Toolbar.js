...
var Toolbar = (_temp = _class = function (_Component) {
  ...
  _createClass(Toolbar, [{
    ...
    value: function toggleBlockType(blockType) {
      ...
      var selectedBlockMap = (0, _utils.getSelectedBlocksMap)(editorState);
      var newBlockMap = selectedBlockMap.map(function (block) {
        if (block.get("type") !== "atomic") {
          return block.set("type", blockType);
        }
        return block;
      });
      var newContentState = currentContentState.set("blockMap", newBlockMap);
      var newEditorState = _draftJs.EditorState.push(editorState, newContentState, "change-block-type");
